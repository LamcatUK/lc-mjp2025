...
# Get all block files
blocks_dir="./blocks/"
block_files=($(ls -1 ${blocks_dir}cb-*.php 2>/dev/null | sort))
...
block_name=$(echo "$block_kebab" | sed 's/cb-//' | sed 's/-/ /g' | awk '{for(i=1;i<=NF;i++) $i=toupper(substr($i,1,1)) tolower(substr($i,2));}1')
